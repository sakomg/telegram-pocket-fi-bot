...
function parsePaletteVariant(envs) {
  ...
  const isVariantValid = validPaletteVariants.includes(
    cliPaletteVariantRaw.toUpperCase()
  );
  if (!isVariantValid) {
    return "LIGHT";
  }
...
function parseApplicationEnvs() {
  ...
  const SHOW_LOGS_MESSAGES =
    process.env.SHOW_LOGS_MESSAGES !== undefined
      ? Boolean(Number(process.env.SHOW_LOGS_MESSAGES))
      : true;

  const CONTINUOUS_RUN_MODE = process.env.CONTINUOUS_RUN_MODE === "1";
  const CONTINUOUS_RUN_MODE_TIMEOUT_MINS = parseContiniousModeTimeouts(
    process.env
  ) || [20, 30];
